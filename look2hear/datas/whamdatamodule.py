...
class WhamDataset(Dataset):
    ...
    def preprocess_audio_only(self, idx: int):
        if self.n_src == 2:
            ...
            # Load noise 
            n, _ = sf.read(
                self.noise[idx][0], start=rand_start, stop=stop, dtype="float32"
            )
            # Load sources
            source_arrays = []
            ...
            noise_tensor = torch.from_numpy(n)
            ...
            if self.normalize_audio:
                ...
                noise_tensor = normalize_tensor_wav(noise_tensor, eps=self.EPS, std=m_std)

            return mixture, sources, noise_tensor, self.mix[idx][0].split("/")[-1]
