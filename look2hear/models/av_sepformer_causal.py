...
class CrossTransformer(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        seq_len = x.size(1)
        src_mask = nn.Transformer.generate_square_subsequent_mask(seq_len, device=x.device)
        for layer in self.transformer_layers:
            x = layer(x, src_mask=src_mask, is_causal=True)
        return x
...
class SBTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_positional_encoding:
            ...
            mask = nn.Transformer.generate_square_subsequent_mask(seq_len, device=pos_enc.device)
            x = self.mdl(pos_enc, mask=mask, is_causal=True)
        else:
            #x = self.mdl(x)
            seq_len = x.size(1)
            mask = nn.Transformer.generate_square_subsequent_mask(seq_len, device=x.device)
            x = self.mdl(x, mask=mask, is_causal=True)
...
class Cross_Sepformer(BaseModel):
    def __init__(self,
                 kernel_size=16,
                 N_encoder_out=256,
                 num_spks=1,
                 sample_rate=16000):
        super(Cross_Sepformer, self).__init__(sample_rate=sample_rate)
        ...
        self.InterSeparator = CrossTransformerBlock(
            num_layers=8,
        ...
        self.IntraSeparator = SBTransformerBlock(
            num_layers=8,
            d_model=N_encoder_out,
            nhead=8,
            d_ffn=1024,
        ...
        self.Separator = Cross_Dual_Path_Model(num_spks=num_spks,
                                               ...
                                               num_layers=2,
                                               K=160,
                                               intra_model=self.IntraSeparator,
                                               inter_model=self.InterSeparator,
                                               norm='ln',
                                               skip_around_intra=True)
        self.num_spks = num_spks

    def forward(self, audio_mixture, mouth_embedding):
        mix_w = self.AudioEncoder(audio_mixture)

        est_mask = self.Separator(mix_w, mouth_embedding)
        ...
        return est_source.permute(0, 2, 1)
    

class AV_Sepformer_Causal(BaseModel):
    def __init__(self, 
                 kernel_size=16, 
                 N_encoder_out=256, 
                 num_spks=1, 
                 sample_rate=16000):
        super(AV_Sepformer_Causal, self).__init__(sample_rate=sample_rate)
    
        # Initialize the Cross_Sepformer
        self.model = Cross_Sepformer(
            kernel_size=kernel_size,
    ...
    def forward(self, audio_mixture, mouth_embedding):
        return self.model(audio_mixture, mouth_embedding)
